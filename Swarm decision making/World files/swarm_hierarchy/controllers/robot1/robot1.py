...
def run_robot(robot):
    ...
    def update_opinion_quality():
        ...
        if(value<400):
            if(opinion=="black"):
                ...
                opinion_quality=opinion_quality*samples_collected/(samples_collected+1)
        print(name,":","Data collected, new opinion quality:", opinion_quality)
        samples_collected+=1
    ...
    def spread_opinion(time):
        ...
        if(broadcasting==False):
            print(name,"current opinion quality:", opinion_quality)
            print(name,":","start dissemination","opinion:",opinion,", currenttime",robot.getTime())
    ...
    def update_opinion():
        ...
        if(collecting==False):
            print(name, ": start opinion collection")
            collecting=True
    ...
    def explore_phase():
        ...
        if(exploring==False):
            print(name, "start exploring",robot.getTime())
            exploring=True
    ...
    def update_time_functions():
        ...
        if(broadcastStartTime!="x"):
            if(robot.getTime()-broadcastStartTime>=broadcast_time):
                print(name, "finish dissemination")
                broadcasting=False
                broadcastStartTime="x"
        ...
        if(collectionStartTime!="x"):
            if(robot.getTime()-collectionStartTime>=7):
                print(name, "finish polling, nothing collected")
                collecting=False
                collectionStartTime="x"
                receiver.disable()
                state2="exploring"
                stable_cycles+=1
                cycle+=1
                print(name, "cycle", cycle, "completed, stable cycles:", stable_cycles)
                if(stable_cycles>8):
                    right_motor.setVelocity(0)
                    left_motor.setVelocity(0)
                    print(name, "opinion stabilised as", opinion, "time:", robot.getTime())
                    exit()
            elif(robot.getTime()-collectionStartTime>=3 and len(opinions)>0):
                print(name, "finish opinion collection, collected:", opinions)
                collecting=False
                collectionStartTime="x"
                receiver.disable()
                old_opinion=opinion
                ...
                opinions=[]
                state2="exploring"
                print(name, "new opinion:", opinion)
                cycle+=1
                print(name, "cycle", cycle, "completed, stable cycles:", stable_cycles)
                if(stable_cycles>8):
                    right_motor.setVelocity(0)
        ...
        if(exploreStartTime!="x"):
            if(robot.getTime()-exploreStartTime>=18):
                print(name, "finish exploring, currenttime",robot.getTime())
                exploring=False
                exploreStartTime="x"
                state2="broadcasting"
    ...
    while robot.step(timestep) != -1:
        ...
        if(state2=="exploring"):
            ...
            irsensor.disable()
            broadcast_time = 2+(opinion_quality*8) #write modulation code but this is a placeholder
            spread_opinion(broadcast_time)#write modulation function later
        else:
            update_opinion()
